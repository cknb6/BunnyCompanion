...
| 当前主版本 | **1.4.x**（Release 标签 `v1.4.0.<run>`，含补丁号便于自动更新） |
...
| **本机工具 30+** | 定位、天气预警、备忘、星座/农历生日、今日卡、列目录、读写移动复制删除、搜索、PowerShell、剪贴板、特殊文件夹、进程、系统监控、抓网页、搜索、浏览器标签、技能插件、**语音朗读** 等 |
| **工具循环** | 标准 function calling；支持多工具并行；模型若把工具写成伪 XML 标签也会本地解析执行并清洗，不把乱码甩给用户 |
| **路径别名** | `桌面` / `Desktop` / `文档` / `下载` / `~/Desktop` 等，不必猜盘符 |
| **附件** | 「＋」选择 · **拖进窗口**（含微信图片虚拟文件/预览图）· Ctrl+V 粘贴 |
| **语音** | 🎤 语音输入（识别成功自动发送）· 朗读回复（可关） |
| **记忆** | `companion_memory.json` 人物/偏好/备忘 + `agent.md` 滚动摘要（仅本机） |
| **预取** | 定位/天气等意图本地先取真数据，减少模型瞎编 |
