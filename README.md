...
- **系统监控触发器**：CPU/内存过高、低电量、久坐离开 → 桌宠自动提醒并配动作
- **语音能力**：聊天窗麦克风语音输入 + 朗读回复。朗读优先在线真人级音色，断网自动回退系统语音；界面只显示「语音输入/朗读」，不暴露任何接口名。可在设置开关。
...
- **本机工具**（30+ 个，真实操作 Windows，非假装）：定位 / 天气预警 / 备忘提醒 / 星座 / 今日卡 / 列目录 / 读写移动复制删除文件 / 搜索 / PowerShell / 打开路径或网址 / 剪贴板 / 特殊文件夹 / 进程列表 / 系统信息 / **系统监控（CPU/内存/电池/闲置）** / **抓网页正文 / 搜索 / 读浏览器标签**等。删除与命令有高危护栏，系统关键路径拒绝操作。
- **技能插件系统**：在 `%LocalAppData%\BunnyCompanion\skills\` 放 Markdown 技能文件（带 frontmatter 描述+触发词+可选命令），Agent 可列出、读取并执行。内置清理临时文件、今日待办、打开常用等示例技能，可自由扩展。
- **长期记忆**（双层，均只存本机）：
