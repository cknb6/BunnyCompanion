...
- **系统监控触发器**：CPU/内存过高、低电量、久离返回 → 桌宠自动提醒并配动作；阈值、冷却时间和总开关都可在设置中调整
- **语音能力**：聊天窗麦克风语音输入 + 朗读回复。朗读优先在线真人级音色，断网自动回退系统语音；界面只显示「语音输入/朗读」，不暴露任何接口名。可在设置开关。
- **更顺手的 Agent 聊天**：一次可发送多张图片并同时附带桌面截图；生成期间可点「停止」或按 Esc 取消；最小化后可用托盘/快捷键恢复
...
- **本机工具**（30+ 个，真实操作 Windows，非假装）：定位 / 天气预警 / 备忘提醒 / 星座 / 今日卡 / 列目录（支持「桌面/文档/下载」别名）/ 读写移动复制删除文件 / 搜索 / PowerShell / 打开路径或网址 / 剪贴板 / 特殊文件夹 / 进程列表 / 系统信息 / **系统监控（CPU/内存/电池/闲置）** / **抓网页正文 / 搜索 / 读浏览器标签**等。删除与命令有高危护栏，系统关键路径拒绝操作。
- **聊天附件**：点「＋」选择，或把文件拖进聊天窗口，或 Ctrl+V 粘贴截图/文件（图片与文本/代码，最多 6 个）。
- **自动更新**：托盘「检查更新」或启动自动检查；仅从本仓库 GitHub Release 下载，并强制用 `checksums.txt` 的 **SHA256** 校验，不一致则拒绝安装。
...
- 支持一次发送多张图片，并可同时附带桌面截图；实际发送内容与界面提示保持一致。
- 生成回复时，“发送”按钮会切换为“停止”，也可按 `Esc` 随时取消；取消后不会留下未回答的错误上下文。
- 图片读取和压缩改为后台处理，大图或多图不再长时间卡住聊天窗口。
- 修复离线状态被立即覆盖、最小化后快捷键无法恢复、关闭旧聊天后新旧请求互相影响等问题。

### 语音与系统关怀

- 设置页新增“朗读回复”“语音输入”“系统状态提醒”入口。
- CPU、内存、低电量、久离时间和提醒冷却均可自行调整，填写 `0` 可关闭对应提醒。
- 久离提醒改为在用户回到电脑后出现；修复电池百分比被错误换算、系统内存读取失败时误报 100% 等问题。
- 修复语音识别与本机命令可能不执行、超时失效或停止后仍卡住的问题。

### 窗口与多显示器

- 聊天和设置窗口会按当前显示器工作区自动适配，小屏和高 DPI 环境下仍可操作底部按钮。
- 修复跨不同缩放比例显示器拖动时的位置跳变。
- 全屏自动隐藏只影响桌宠所在显示器；副屏桌宠不会再被主屏全屏程序误隐藏。
- 桌面截图会正确选择桌宠所在显示器，快捷键冲突也会在帮助中明确提示。

### 工具与发布可靠性

- 本机命令和技能命令支持真实取消、超时终止、带空格的程序路径及正确的 PowerShell 版本选择。
- 网页抓取增加 URL 校验、返回长度与响应缓冲上限，降低错误链接或超大页面导致的异常。
- 发布流程会分别生成并核验 x64、ARM64 的 SHA256，避免一个架构的校验信息覆盖另一个架构。
- GitHub Actions 发布前会运行离线词库、核心逻辑、XAML/素材结构校验；测试工具与主程序统一使用 .NET 8。
- 当前版本号为 **1.3.0**，程序“关于”、本地校验文件和 Release 标签均从工程版本统一生成。
...
- 发布前自动执行离线词库、核心逻辑、XAML/素材结构校验，并核对两个架构的 SHA256
...
- 仅在打开 Agent 聊天并走在线通道时联网；断网自动切换本地陪伴。
- 看桌面需主动触发，不后台偷拍。
...
本仓库当前未授予开源许可，源码、角色素材与截图均保留所有权利。转载、二次分发、商用或制作衍生版本前，请先联系传康KK取得授权。
